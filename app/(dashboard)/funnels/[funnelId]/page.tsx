...
export default function FunnelEditor() {
  ...
  const [confirmDelete, setConfirmDelete] = useState<{ isOpen: boolean; stageId?: string; stageName?: string }>({
    isOpen: false
  });
  ...
  const openDeleteConfirm = (stageId: string, stageName: string) => {
    setConfirmDelete({
      isOpen: true,
      stageId,
      stageName
  ...
  return (
    <div className={styles.container}>
              ...
              {stages.map((stage, index) => (
                <React.Fragment key={stage.id}>
                  <StageCard
                    ...
                    onDelete={(stageId) => openDeleteConfirm(stageId, stage.name)}
      ...
      <ConfirmDialog
        ...
        message={`Tem certeza que deseja excluir a etapa "${confirmDelete.stageName}"?\n\nEsta ação não pode ser desfeita e todos os dados relacionados serão perdidos.`}
        confirmText="Excluir"
        cancelText="Cancelar"
        onConfirm={() => {
          if (confirmDelete.stageId) {
            handleStageDelete(confirmDelete.stageId);
          }
        }}
